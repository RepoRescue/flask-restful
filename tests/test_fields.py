...
def test_float():
    ...
    for value, expected in values:
        assert expected == fields.Float().output('a', {'a': value})
...
def test_boolean():
    ...
    for value, expected in values:
        assert expected == fields.Boolean().output('a', {'a': value})
...
def test_rfc822_datetime_formatters():
    dates = [
        ...
        (datetime(2011, 1, 1, 23, 59, 59, tzinfo=pytz.utc),
         "Sat, 01 Jan 2011 23:59:59 -0000"),
        (datetime(2011, 1, 1, 23, 59, 59, tzinfo=timezone(timedelta(hours=1))),
...
class FieldsTestCase(unittest.TestCase):
    ...
    def test_rfc822_date_field_with_offset(self):
        obj = {"bar": datetime(2011, 8, 22, 20, 58, 45, tzinfo=timezone(timedelta(hours=1)))}
        field = fields.DateTime()
